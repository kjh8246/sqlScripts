...
-- (명령어 한개실행은 ctrl+enter , 여러명령 한번에 실행은 드래그 하여 명령선택하고alt+x)
...
-- 데이터 조회 할때 컬럼명 대신에 함수를(count,sum,avg,max,min)를 사용
...
-- 데이터 생단위 삭제 : delete FROM 테이블명 where 컬럼명1=값1 and 컬럼명2=값2,...
--										where 삭제할 행에 대한 조건
